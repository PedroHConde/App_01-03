...
byte msg[6];  //Mensagem a ser recebida
...
void loop() {
  ...
  ///msg=[0,tarefa,dado1,dado2,dado3,1]

  // ----------------------------   Lê porta serial para receber comandos   ---------------------------
  //comandoCompleto = false;
  //void serialEvent() {
  if (Serial.available() > 5) {  // Verifica se há pelo menos 6 bytes disponíveis,
                                 //Verificando se a mensagem chegou inteira
                                 //0 - Start Bit
                                 //1 ... 4 - tarefa
                                 //5 ... 8 - valor1
                                 //9 ... 12 - valor2
    ...
    for (int i = 0; i < 6; i++) {
      msg[i] = Serial.read();  //Armazenando mensagem recebida
      //digitalWrite(13, HIGH);
    }

    if (msg[0] == 0) {  // Byte de início

      if (msg[5] == 1) {  // Verifica se o bit de fim está correto
        ...
        //     primeiroCaractereComando = true;
            ...
            if (!degrau) {  // Degraus ascendentes e descendentes
              ...
              duracaoDegrau = (int)msg[2];
              numDegraus = (int)msg[3];

              amplitudeDegrau = (float)(TmaxDegrau - TminDegrau) / (numDegraus - 1);
            }
...
//         }//fim switch
  ...
  if (atualizacao && malhaFechada) {
    if (degrau) {
      //Serial.print("entrei");                                               // Degraus ascendentes e descendentes
